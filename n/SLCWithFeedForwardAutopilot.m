...
%   3/20/2017 - EWF created from SimpleSLCAutopilot
%
%
% uu input vector is 
...
%            x_command]
    ...
    h_dot_c         = uu(16,1);  % commanded altitude (m)
    h_c     = uu(17,1);  % commanded altitude rate (m)
    chi_dot_c       = uu(18,1);  % commanded course (rad)
    chi_c   = uu(19,1);  % commanded course rate (rad)   
    Va_c        = uu(20,1);  % commanded airspeed (m/s)
...
%         chi = chi_c;
    ...
    %----------------------------------------------------------
    % lateral autopilot
    chi_dot_des = control_gain_struct.Kff_course_rate*chi_dot_c + control_gain_struct.Kp_course_rate*(unwrap_angle(chi_c - flight_angles(2,1)));    
    %chi_dot_des = Va/500;
    
    [phi_des, q_des, r_des] = coordinated_turn_rates(chi_dot_des, Va, control_gain_struct);
...
%   coordinated turn
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [phi_des, q_des, r_des] = coordinated_turn_rates(chi_dot_des, V, control_gains)

    phi_des = atan2(chi_dot_des*V, control_gains.g);
    phi_des = sat(phi_des, control_gains.max_roll, -control_gains.max_roll);

    q_des = chi_dot_des*sin(phi_des);
    r_des = chi_dot_des*cos(phi_des);
end
...
function [delta_a, p_c] = roll_hold(phi_c, phi, p, flag, control_gains)
  ...
  % implement PI control for roll rate
...
delta_e = sat(delta_e_trim+uff+up+ud, control_gains.max_de, -control_gains.max_de);
